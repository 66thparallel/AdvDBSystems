...
Classes:
    AccessType: Indicates the type of transaction.
    Transaction: Describes each transaction--their id, time stamp, the type
        of transaction (read only, write, etc.), the location where they are
        stored, the locks currently held, and whether the transaction has
        aborted and why.
    ReadOnlyTransaction: Inherits from Transaction class. Indicates that
        the transaction is read only.
    TestTransaction: Unit tests for the Transaction class.
    TestReadOnlyTransaction: Unit tests for the ReadOnlyTransaction.
...
class TestTransaction(unittest.TestCase):
    def setUp(self):
        self.t = Transaction(3, 20)
        self.t._accessed_sites = {(2, 5)}
        self.t.fail_site(4)
        self.t._rlocks = {(6, 5)}
        self.t._wlocks = {(8, 7)}
