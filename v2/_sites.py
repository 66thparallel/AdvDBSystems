'''
    SiteEntry holds 10 or 12 variables. For each of those vars there can be many versions. Some are ready to read while
    others aren't. Makes sense to have a way to track vars--their old and new values. A site holds 10 or 12 vars but
    all indexed 1-20. SiteEntry is one of those variables. SiteEntry has methods __new__(), which controls objects
    creation. __init__(). When Site is created, there's 20 vars. Normal vars is 0 - 19 except when reading from
    command line. Override __getitem__().
    Ex.
    s3 = Site(2) # this will print out site 3.
    __new__() checks if the var is valid for that site. returns a variable of its class. returns none if var is odd and not at correct index.
    Ex.
    SiteEntry(8, 4)
    8 is zindex and returns None. 1 will be added to 8 in __new__() and if it returns instance of class that should be created, then python
     calls _init__() on that object.
    Ex. (this is what interpreter is seeing:
    ob = SiteEntry.__new__(SiteEntry, )
    instanceof(ob, SiteEntry):
'''
import logging
import unittest
...
class Site(object):
    ...
    def read(self, var, txn):
        """
            Check if transaction is read only. Gets lock and performs read. If
